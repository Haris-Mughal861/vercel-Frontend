...
const SellerForgotPassword = () => {
  ...
  const sendCode = async (email) => {
    const data = {
      url: apis().sellerRequestResetCode,
      method: 'POST',
      body: { email }
    };
  ...
  const resetPassword = async (values) => {
    const data = {
      url: apis().sellerVerifyResetCode,
      method: 'POST',
      body: {
  ...
  return (
    <div className="auth_main">
      <Paper elevation={4} className="auth_card">
        <Typography variant="h5" fontWeight="bold">Seller Forgot Password</Typography>
        ...
        <Formik
          ...
          validationSchema={Yup.object({
            ...
            password: codeSent ? Yup.string().min(6, 'Min 6 chars').required('Required') : Yup.string()
          })}
          onSubmit={(values) => {
            if (!codeSent) {
